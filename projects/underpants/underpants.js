...
_.last = function(arr, num){
    var lastArr = []; // creating array literal 
    if(!Array.isArray(arr) || num < 0){ // if arr is not an array
        return [];
    }else if(typeof num !== 'number'){ // if num is not a number
        return arr[arr.length -1]; // return last element in arr
     
        }else if( num > arr.length){
            return arr;
        }
        return arr.splice(arr.length - num);// otherwise 
...
_.partition= function(arr,func){
    let all = []; //litearl array to contain both functions. 
   let pass = _.filter(arr,func); // our pass function
   let fail = _.reject(arr,func);// our fail function
   all.push(pass); ////pass goes into all array
   all.push(fail); // fail goes behind pass in all array
   return all; // returns all array full of both results 
}
/** _.map
* Arguments:
*   1) A collection
...
_.map = function(collection,func){
    let mixed = []; //literal array 
         _.each(collection, function(e,i,collection){ //function that loops 
            mixed.push(func(e,i,collection))
...
_.pluck = function(arr,property){
     ...
     let values = _.map(arr,function(object,i,arr){
...
_.every = function(collection,func){
    let eV = [];
    let nV = [];
    _.each(collection, function(object,i,collection){
        let t = func(object,i,collection);
        ...
        if (eV.length === collection.length){
            return true;
        }else{
            return false;
        }
